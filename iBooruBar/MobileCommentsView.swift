...
struct MobileCommentsView: View {
    ...
    private func commentBody(_ body: String) -> some View {
        if site.apiType == .philomena {
            philomenaSegmentsView(
                PhilomenaCommentBodyParser.parse(body),
                visitedCommentIDs: [],
                depth: 0
            )
        } else {
    ...
    private func philomenaSegmentsView(
        _ segments: [PhilomenaCommentBodySegment],
    ...
    private func philomenaSegmentView(
        _ segment: PhilomenaCommentBodySegment,
        visitedCommentIDs: Set<Int>,
        depth: Int
    ) -> AnyView {
        switch segment {
    ...
    private func philomenaReplyView(
        ...
        return philomenaQuoteView(
            ...
            commentID: commentID,
            segments: PhilomenaCommentBodyParser.parse(referencedComment.body),
    ...
    private func philomenaQuoteView(
        author: String?,
        commentID: Int?,
        segments: [PhilomenaCommentBodySegment],
        visitedCommentIDs: Set<Int>,
        depth: Int
    ) -> AnyView {
        ...
        if renderedSegments.isEmpty,
           ...
           let referencedComment = comments.first(where: { $0.id == commentID }) {
            ...
            renderedSegments = PhilomenaCommentBodyParser.parse(referencedComment.body)
            nextVisited.insert(commentID)
        }
        ...
        return AnyView(
            VStack(alignment: .leading, spacing: 7) {
                if let renderedAuthor, !renderedAuthor.isEmpty {
                    Text(renderedAuthor)
                        .font(.subheadline.weight(.bold))
                        .frame(maxWidth: .infinity, alignment: .leading)
...
private indirect enum PhilomenaCommentBodySegment: Equatable {
    case text(String)
    case quote(author: String?, commentID: Int?, segments: [PhilomenaCommentBodySegment])
    case reply(author: String, commentID: Int)
}

private enum PhilomenaCommentBodyParser {
    private struct Line {
        let depth: Int
        let text: String
    }
    ...
    static func parse(_ body: String) -> [PhilomenaCommentBodySegment] {
        let normalized = body
    ...
    private static func parseLevel(
        ...
        index: inout Int,
        level: Int
    ) -> [PhilomenaCommentBodySegment] {
        var segments: [PhilomenaCommentBodySegment] = []
    ...
    private static func mergeReplyWithQuotedSnapshot(
        ...
        var result: [PhilomenaCommentBodySegment] = []
        var index = 0
    ...
    private static func replyReference(_ text: String) -> (author: String, commentID: Int)? {
        guard let replyExpression else { return nil }
